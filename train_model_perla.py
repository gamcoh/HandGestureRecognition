...
model.add(keras.layers.Conv2D(18, (4, 4), activation='relu')) 
model.add(keras.layers.MaxPooling2D((2, 2)))
...
checkpoint = keras.callbacks.ModelCheckpoint("perla.h5", monitor='val_accuracy', verbose=1, save_best_only=True, save_weights_only=False, mode='auto', period=1)
early = keras.callbacks.EarlyStopping(monitor='val_accuracy', min_delta=0, patience=40, verbose=1, mode='auto')
model.fit_generator(train_generator, steps_per_epoch=200, epochs=500, verbose=1, validation_data=val_generator, validation_steps=10, callbacks=[early, checkpoint, lr_schedule])
